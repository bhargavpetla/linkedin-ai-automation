...
process.env.GEMINI_API_KEY = 'test-key'; // Mock key to bypass constructor check
...
#AI #MachineLearning #DeepLearning #AIResearch #TechInnovation #DeepSeek #ModelArchitecture`;
    ...
    const mockAnalysis = {
        mainTopic: "DeepSeek's MHC: Smarter AI 'Highways' for Scalability",
        subTopics: ["ResNet shortcuts", "Hyperconnections chaos", "MHC controlled flow"],
        keyMetrics: [{ label: "Signal Amplification", value: "1.6x", trend: "neutral" as const }],
        keyPoints: [
            "ResNet used single shortcuts to keep signals stable",
            "Hyperconnections (HC) added multiple paths but caused chaos (3000x amp)",
            "MHC adds mathematical guardrails to control flow (1.6x stable)"
        ],
        tone: "educational" as const,
        contentType: "insight" as const,
        techKeywords: ["DeepSeek", "MHC", "Hyperconnections", "ResNet"],
        visualConcept: "Comparison of 3 network architectures"
    };

    const prompt = imageService.buildBhargavArchitectPrompt(post, mockAnalysis);
    console.log("\n--- GENERATED PROMPT ---");
    console.log(prompt);
    console.log("------------------------\n");
}
